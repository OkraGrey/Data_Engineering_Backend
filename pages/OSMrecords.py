...
from Data_Engineering_Backend.App import wrapper
...
def get_all_amenities():
    try:
        conn = get_db_connection()
        cursor = conn.cursor()
        cursor.execute("SELECT key_name FROM osm_keys ORDER BY key_name ASC;")
        rows = cursor.fetchall()
...
with st.container():
    ...
    with col2:
        ...
        # Set default value to "" (the first option)
        keyword = st.selectbox("Keys", [""] + amenities, index=0)
...
if search_clicked:
    ...
    if not county_stripped and not keyword_stripped:
        ...
        with st.spinner("Fetching data from backend..."):
            try:
                data = wrapper(county_stripped, keyword_stripped)
                if data:
